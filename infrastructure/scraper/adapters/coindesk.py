...
class CoindeskAdapter(SiteAdapter):
    ...
    def fetch_listing_urls(self):
        r = requests.get(self.RSS, timeout=15, headers=HEADERS)
        r.raise_for_status()
        soup = BeautifulSoup(r.text, "xml")

        seen = set()
        for item in soup.find_all("item"):
            href = item.link.get_text(strip=True)
    ...
    def parse_article(self, url: str):
        ...
        title = (
                ...
                ) or (soup.select_one("h1").get_text(strip=True) if soup.select_one("h1") else None) or canonical_url
        ...
        author = author_el.get("content") if author_el and author_el.has_attr("content") else (
